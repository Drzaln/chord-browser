...
| **Tests**                        | **751 passing** (`swift test`, 108 suites), measured 2026-09-17                                                                                                                                |
| **Schema**                       | **v15** — … `v12_credentials`, `v13_credential_never_save`, `v14_tab_custom_title`, `v15_window_blank_spaces`                                                                                                      |
...
**Settings → Updates** shows the installed version and checks on open; if the
latest tag outranks the installed version (SemVer, `v` prefix tolerated,
prereleases never offered) it offers **Download & Install**, which downloads
the zip, extracts it with `ditto` (preserves the bundle's
symlinks/permissions), and swaps the `.app` into `/Applications` — replacing a
running app is safe on macOS, and the quarantine attribute is stripped so
Gatekeeper does not flag the update the user just asked for. Relaunch is
deferred: a detached helper waits for this process to exit (so
`applicationShouldTerminate` flushes session state) and then `open -n`s the new
app through LaunchServices.

**2026-08-22 hardening.** The version check no longer touches the GitHub REST
API — `api.github.com/releases/latest` caps unauthenticated callers at 60
requests/hour per IP, and the app hit the wall (a bare "HTTP 403", verified
`X-RateLimit-Remaining: 0`). `GitHubReleaseChecking` now follows the public web
URL `github.com/<repo>/releases/latest`, which 302-redirects to the tag page, and
reads the version from the `Location` header — no API, no limit. The zip is
fetched from the stable `releases/latest/download/<asset>` link, so the download
URL needs no version number either. Automatic checks are throttled to one per 5
minutes (explicit **Check for Updates** forces through). The relaunch helper was
also made self-healing: it sweeps stale Chord instances, waits ~5 s for its own
PID to exit, force-kills it if the async shutdown hangs, then opens the new app
— earlier versions left a second, orphaned Chord in the dock (two-dock bug, seen
on 1.4.0→1.4.1 and fixed by 1.4.3).

Everything lives in a new Foundation-only **`ChordUpdater`** package (`Version`
semver type, `GitHubRelease` decoding, `GitHubReleaseChecking` over the web
redirect, `AppInstaller`, and a `@MainActor @Observable UpdateController` that
drives the Settings UI). The UI layer alone imports AppKit, for relaunch/terminate — the
...
## Blank window persists across relaunch (2026-09-17)

**Behavior.** Arc keeps a window blank across a quit/relaunch when it was left
blank (the last live favourite/Pinned closed with nothing to focus) and
re-offers the command bar instead of reviving a tab. Chord used to reconcile the
empty selection back to the MRU tab on restore.

**The fix.**
- `TabStore+Windows.applyLayout`: a saved `selectedTabID == nil` is now
  honoured — the window stays blank — instead of falling through to `reconcile`.
  A *non-nil* id that no longer exists still reconciles, so the stale-tab case is
  unchanged.
- `applyRestoredLayouts` (primary/early windows) and `claimWindow` (scenes macOS
  restores after launch) call `closeLeftBlankPresenter` for a blank window, so
  the command bar appears on launch exactly as it does when the close happens
  live.
- `TabStore+Spaces.selectSpace`: a **blank** window stays blank when the Space
  changes — the old path revived `lastSelectedTabBySpace`/the MRU tab. The bar is
  re-offered, and `ChordApp`'s presenter now skips when the bar is already
  visible, so switching Space does not reset a query the user is typing.

**Tests.** `blankWindowRestoresBlank`, `blankSecondWindowRestoresBlank`
(MultiWindowTests), `blankWindowStaysBlankOnSpaceSwitch` (SpaceTests) — **738
total, green**.

## Command bar tinted with the Space gradient (2026-09-17)

The bar was a neutral `.regularMaterial` panel, so it read as system chrome while
everything around it (the sidebar, the blank content card) carried the active
Space's colour. It now paints `SpaceTheme.gradient(for:)` at
...
`RootView.toastText` and `ToastBanner`.

**Applied to every 0.40-tinted surface**, via `SpaceTheme.foregroundPair(on:…)`
which returns a `primary` + dimmer `secondary` pair: a **selected sidebar row**
(`TabRowView`, so ephemeral and Pinned rows both), a **selected favourite tile's**
fallback icon (`PinnedGrid`), and the **picked command-bar row**
(`CommandBarRow`). Weak-wash surfaces (the 0.1–0.28 sidebar/bar backgrounds) stay
on `.primary`/`.secondary`, which already adapt to light/dark. Tests:
`ToastTextContrastTests` — **745 total, green**.

## Blank is remembered per Space (2026-09-17)

**The bug.** Blank was only `selectedTabID == nil`, a **per-window** fact. Blank
both Spaces, open a tab in one, and switching to the other loaded its last tab;
with two windows, a tab opened in one revived the blank other through `reconcile`.

**The fix.**
- `WindowState.blankSpaceIDs`: the Spaces this window left blank (Arc's new-tab
  state is per-Space).
- `selectSpace` records the departing Space (its tab, or blank) and, on entry,
  stays blank **iff that Space is marked blank** — purely per-Space. A Space
  holding a tab always comes back to it. (An earlier cut also stayed blank when
  the *window* was blank, which leaked a blank Space's state onto the one being
  entered: leaving YouTube Music in Space 1, visiting a blank Space 2, then
...
- Persisted (v15): `WindowLayout.blankSpaceIDs` rides in the `windowLayout`
  `blankSpaceIds` column (sorted UUID strings, comma-joined; NULL when empty), so
  blank survives a relaunch for **every** Space, not just the active one. Capture
  drops a Space's mark if it is showing a tab.

**Tests.** `blankIsPerSpaceOnSwitch`, `returningRestoresTab`,
`blankSpaceSurvivesActivityElsewhere` (SpaceTests),
