...
/// `ChordEngine`'s opaque `ExtensionControllerHandle`, never a raw `WK*`
...
public protocol ExtensionHost: AnyObject, ExtensionControllerProviding {
    ...
    /// `ChordStore` wires this callback and imports no AppKit at all; naming
    /// `NSWindow` here would drag AppKit into the Store to satisfy the closure's
    /// type. `ChordUI` — which is AppKit either way — casts it back and
