...
struct SidebarView: View {
    ...
    private func gradient(for space: ChordCore.Space) -> LinearGradient {
    ...
    /// The Space colour that tints the sidebar: the (blend-aware) gradient at a
    /// low opacity. Floating uses a lower tint so the page reads through it;
    /// docked keeps more so the Space colour still carries.
    @ViewBuilder
    private var sidebarTint: some View {
        if let space = store.activeSpace(in: windowState) {
            gradient(for: space).opacity(isFloating ? 0.1 : 0.28)
        } else {
            Color.clear
        }
    }
    ...
    @ViewBuilder
    private func rowMenu(for tab: ChordCore.Tab) -> some View {
