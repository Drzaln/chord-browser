...
///
/// This is the half that is safe to keep in `chord.sqlite`: which site, which
/// username, when it was last used. **The secret never appears here** — it lives
/// in the Keychain, reachable only through `ChordSecrets`, joined to this by
...
///
/// Pure and in `ChordCore` so the rule can be tested exhaustively with no
