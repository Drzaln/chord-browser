...
/// vault). The secret half is `ChordSecrets.SecretStore`, joined by
/// `Credential.id`.
///
/// Declared here in `ChordCore`, WebKit-free and Foundation-only, so the Store
/// can depend on the shape and `ChordPersistence` can implement it — the same
