...
public final class TabStore {
    ...
    public init(engine: any WebEngine, repository: any TabRepository, clock: any Clock) {
        self.engine = engine
    ...
    public func restore() async {
        ...
        if tabs.isEmpty {
            ...
            selectedTabID = tabs.max { $0.lastAccessedAt < $1.lastAccessedAt }?.id
        }
    }
    ...
    public func newTab(url: URL = TabStore.defaultNewTabURL) {
        let order = (tabs.map(\.placement.order).max() ?? -1) + 1
        let tab = Tab(url: url, placement: .ephemeral(order: order), now: clock.now)
        tabs.append(tab)
    ...
    public func closeTab(_ tabID: UUID) {
        ...
        if selectedTabID == tabID {
            // Select the neighbour that is now in the closed tab's slot.
    ...
    public func surface(for tab: Tab) -> AnyWebSurface {
        engine.surface(for: tab.focusedPane)
    }
