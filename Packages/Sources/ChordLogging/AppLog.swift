...
public enum AppLog {
    ...
    /// `Application Support/<app>/Logs/chord.log`. The file rotates when it
    /// passes `capBytes` (5 MB default): the current file is renamed to
    /// `chord.log.1` (overwriting) and a fresh one is started.
    ...
    static let subsystem = "com.rizal.chord"
...
final class FileSink: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.rizal.chord.log", qos: .utility)
