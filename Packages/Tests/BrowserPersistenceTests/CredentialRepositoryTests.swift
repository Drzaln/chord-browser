...
/// `BrowserSecrets`, and the fact that these tests never mention one is the point.
@Suite("Credential repository")
struct CredentialRepositoryTests {

    private func makeRepository() throws -> SQLiteCredentialRepository {
        SQLiteCredentialRepository(database: try BrowserDatabase.inMemory())
    }
    ...
    private func makeRepositoryWithSpace() async throws
        -> (SQLiteCredentialRepository, BrowserCore.Space)
    {
        let database = try BrowserDatabase.inMemory()
        let space = BrowserCore.Space(name: "Work", sortIndex: 0)
