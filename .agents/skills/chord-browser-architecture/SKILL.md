...
Packages/Sources/
  ...
  ChordUpdater/        GitHub-release self-updater (check/compare/download/install). Foundation + Observation only.
...
- ~~Close-MRU blank end~~ **Done 2026-09-08 (1.9.0)** — closing a favourite/Pinned
  tab with an empty recency stack leaves the window **blank** (nothing rendered,
  Arc behaviour) and offers the command bar via `closeLeftBlankPresenter`
  (app-wired to `CommandBarController`). The slot-neighbour fallback was dropped
